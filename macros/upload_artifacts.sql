...
{% endset %}
{% set put_query %}
    PUT file://{{ filename }} @{{ database }}.dbt_smcintyre.DBT_LOAD auto_compress=true;
{% endset %}
{% set copy_query %}
    BEGIN;
    COPY INTO {{ database }}.dbt_smcintyre.{{ table | upper }} FROM
        @{{ database }}.dbt_smcintyre.DBT_LOAD
        file_format=(type='JSON')
        on_error='skip_file';
    COMMIT;
{% endset %}
{% set remove_query %}
    REMOVE @{{ database }}.dbt_smcintyre.DBT_LOAD pattern='.*.json.gz';
{% endset %}
